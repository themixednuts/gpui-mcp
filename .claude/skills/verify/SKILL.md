...
4. Run `target/debug/gpui-mcp-call --server target/debug/gpui-mcp-server --artifact-dir <private-test-dir> --batch <batch.json>` (append `.exe` to the two binary paths on Windows). The single running demo is selected automatically.
